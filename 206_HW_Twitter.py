...
from twitter_info import consumer_key
from twitter_info import consumer_secret
from twitter_info import access_token
from twitter_info import access_token_secret
...
def getTweetsWithCaching(s):
    
    if s in CACHE_DICTION:
        print("using cache")
        return CACHE_DICTION[s]
    else:
        print("fetching")
        data = api.search(q=s)
        #print (type(data))
...
for i in range(3):
	...
	results = getTweetsWithCaching(search_term)
...
## 		content from 5 tweets, as shown in the linked example.
	...
	for x in results['statuses'][:5]:
		...
		print('CREATED AT: {}'.format(x['created_at']))
		print('\n\n')
